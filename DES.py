...
def shift_left(data: str, shifts: int) -> str:
...
def encrypt(pt, rkb, rk):
    pt = hex_to_bin(pt)
    print(f"Binary message: {pt}")

    pt = pad_data(pt, 64)  

    pt = permute(pt, initial_perm, n=64)
    print(f"Permutated message:", bin_to_hex(pt))
    ...
    for i in range(0, 16):
        right_expanded = permute(right, exp_d, n=48)
        ...
        sbox_str = permute(sbox_str, per, n=32)
    ...
    combine = left + right
    cipher_text = bin_to_hex(permute(combine, final_perm, n=64))
    return cipher_text
...
def generate_round_keys(key):
    ...
    key = permute(key, keyp, n=56)
    print(f"Permutated key: {key}")
    ...
    for i in range(0, 16):
        ...
        round_key = permute(left + right, key_comp, n=48)
        rkb.append(round_key)
        rk.append(bin_to_hex(permute(left + right, key_comp, n=48)))
